#!/usr/bin/env bash
# Start Countdown GRPO training in the BACKGROUND on 4 GPUs:
#   GPU 0        -> vLLM rollout server
#   GPU 1,2,3    -> training (accelerate DDP, 3 processes)
# Survives closing the terminal/browser (tmux session 'cogito' + nohup vLLM).
#
# Env knobs:  MODEL (default Qwen/Qwen2.5-3B), MAXLEN (2048), SMOKE (0/1),
...
echo "[launch] (1/2) starting vLLM rollout server on GPU $VLLM_GPU ..."
MODEL="$MODEL" MAXLEN="$MAXLEN" VLLM_GPU="$VLLM_GPU" bash rollout/serve_vllm.sh

CMD="CUDA_VISIBLE_DEVICES=$TRAIN_GPUS accelerate launch --config_file configs/accelerate_ddp.yaml --num_processes $NPROC train/grpo_countdown.py --model $MODEL $SMOKE_FLAG"
